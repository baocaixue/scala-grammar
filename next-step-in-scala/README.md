...
　　Scala一直强调的函数式编程，那么如何识别函数式编程呢？    
　　首先是从代码层面识别两种风格的差异。*一个显著的标志是如果代码包含任何var变量，它通常是指令式风格的；而如果代码完全没有var，那么它很可能
是函数式的*。因此一个向函数式风格转变的方向是尽可能不用var。    
　　参考如下这个while循环的例子，使用了var因此是指令式风格的：    
```scala
...
　　不过重构后的printArgs方法并不是“纯”的函数式代码，因为它有副作用（本例中它的副作用是向标准输出流打印）。*带有副作用的函数的标志性特质是结
果为Unit。如果一个函数并不返回任何有意义的值，也就是Unit这样的结果类型所表达的意思，那么这个函数存在于世上的唯一意义就是产生副作用*。函数式编
程的做法是定义一个将传入的args作为格式化的方法，但只是返回这个格式化的字符串，如下所示：    
```scala
def formatArgs(args: Array[String])=args.mkString("\n")
```    

　　现在已经实现了函数式编程：没有副作用，也没有var。     

　　每个有用的程序都会有某种形式的副作用。否则，它对于外部世界没有任何价值。倾向于使用无副作用的函数鼓励设计出带有副作用的代码最小化的程序。Scala
并不是一门纯函数式编程的语言，强制只能用函数式风格来编程。Scala是指令式/函数式混合编程语言。如果发现有些场景下对于要解决问题而言指令式更为合适，
就应该使用指令式风格。总之，优先使用val、不可变对象和没有副作用的方法，不过当有特定的需要和理由，也不必拒绝var、可变对象和带有副作用的方法。    
